...
class JobFixerAgent:
    ...
    async def _comprehensive_scan_and_fix(
        ...
        prompt = (
            f"A Databricks notebook job failed. "
            f"The triggering error is shown below, but you must scan the ENTIRE notebook "
            f"and fix ALL bugs you find in a single pass — not just the one that surfaced first.\n\n"
            f"## Triggering Error\n"
            f"```\n{safe_error}\n```\n\n"
            f"## Notebook Source\n"
            ...
            f"{past_ref_str}"
            f"## What to fix\n"
            f"1. Fix the triggering error above.\n"
            f"2. Scan every cell of the notebook and fix any other bugs that would cause a runtime failure "
            f"(typos in identifiers, wrong function calls, incorrect types, missing/wrong imports, "
            f"logic errors that would raise exceptions). Fix them ALL in this single pass.\n"
            f"3. Do NOT fix speculative or style issues — only things that will cause a crash or incorrect result.\n\n"
            f"## Non-Negotiable Rules\n"
            f"1. NEVER rename variables, functions, table names, or column names.\n"
            f"2. NEVER refactor, restructure, or rewrite working code sections.\n"
            f"3. NEVER add type hints, docstrings, or comments that were not there before.\n"
            f"4. NEVER reformat or restyle lines that are not being bug-fixed.\n"
            f"5. Preserve all Databricks magic comments exactly as-is "
            f"(# Databricks notebook source, # COMMAND ----------, # MAGIC).\n"
            f"6. Only change lines that contain actual bugs. Working code is sacred.\n\n"
            f"## Output Format\n"
            f"Line 1: `FIX: <one sentence listing ALL bugs fixed>`\n\n"
            f"Then the complete corrected notebook source:\n"
            f"```python\n"
            f"[full notebook with all bug-causing lines corrected]\n"
            f"```"
        )

        _base_system = (
            "You are a comprehensive notebook repair tool for production Databricks jobs. "
            "When a job fails, you scan the ENTIRE notebook and fix ALL bugs in a single pass — "
            "not just the one error that surfaced. This is critical: production systems get only "
            "a few retries, so every bug must be caught and fixed upfront. "
            "You fix only genuine bugs (things that cause crashes or wrong results). "
            "You never rename, refactor, restructure, add type hints, or make cosmetic changes. "
            "Working code is left exactly as-is. Only broken lines are touched."
        )
        messages = [
