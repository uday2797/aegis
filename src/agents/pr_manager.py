...
class PRManagerAgent:
    ...
    async def wait_for_pr_approval(self, pr_number: int, timeout_minutes: int = 60) -> Dict:
        """
        Poll PR status until merged or rejected (or timeout).
        ...
        logger.info(f"[PRManager] Waiting for PR #{pr_number} approval (timeout={timeout_minutes}min)")
        ...
        while True:
            try:
                ...
                if elapsed > timeout_seconds:
                    logger.warning(f"[PRManager] PR #{pr_number} approval timeout ({timeout_minutes}min)")
                    return {
                        "merged": False,
                        "closed": False,
                        "sha": None,
                ...
                logger.info(f"[PRManager] PR #{pr_number} still open, waiting... ({elapsed/60:.1f}min elapsed)")
...
### Changes
This PR contains autonomous notebook repairs generated by AEGIS GPT-4o:
