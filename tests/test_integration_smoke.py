...
async def test_run_once_returns_report_or_none(orchestrator, failure_spec):
...
async def test_schema_drift_produces_report(orchestrator):
    orchestrator.inject_failure({"type": "schema_drift"})
    report = await orchestrator.run_once()
    assert report is not None


async def test_transient_failure_produces_report(orchestrator):
    orchestrator.inject_failure({"type": "transient_failure"})
    report = await orchestrator.run_once()
    assert report is not None


async def test_report_has_root_cause(orchestrator):
